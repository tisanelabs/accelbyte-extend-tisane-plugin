...
namespace AccelByte.Extend.Tisane.Plugin
{
    public class AppSettingConfigRepository : IConfigRepository
    {
        ...
        public string ItemIdToGrant { get; set; } = String.Empty;
        ...
        public void ReadEnvironmentVariables()
        {
            ...
            string? itemIdToGrant = Environment.GetEnvironmentVariable("ITEM_ID_TO_GRANT");
            if ((itemIdToGrant != null) && (itemIdToGrant.Trim() != String.Empty))
                ItemIdToGrant = itemIdToGrant.Trim();
        }
    }
}
